// src/kernel/terminal.c - FIXED: Proper framebuffer initialization (UPDATED: scaling + bpp autodetect)
#include "terminal.h"
#include "kstring.h"
#include "io.h"
#include "font_data.h"
...
static int fb_bpp = 0; // bytes per pixel (3 or 4)

// Font settings for framebuffer
static const int FONT_WIDTH = 8;
static const int FONT_HEIGHT = 16;
static int font_scale = 1; // runtime scaling: 1..4
static int cols = 0;
static int rows = 0;
...
static inline int cell_w(void) { return FONT_WIDTH * font_scale; }
static inline int cell_h(void) { return FONT_HEIGHT * font_scale; }
...
static void draw_char_fb(char c, int x, int y, uint32_t fg, uint32_t bg) {
    ...
    const int bpp = (fb_bpp == 4) ? 4 : 3;
    const int scale = font_scale;

    // Draw scaled: each font pixel becomes scale x scale block
    for (int gy = 0; gy < FONT_HEIGHT; gy++) {
        ...
        for (int sy = 0; sy < scale; sy++) {
            ...
            uint8_t* rowp = framebuffer + (py * fb_pitch) + (x * bpp);

            uint8_t b = bits;
            for (int gx = 0; gx < FONT_WIDTH; gx++) {
                uint32_t color = (b & 0x80) ? fg : bg;

                for (int sx = 0; sx < scale; sx++) {
                    int px = x + gx * scale + sx;
                    if (px >= fb_width) break;

                    uint8_t* p = rowp + (px - x) * bpp;
                    p[0] = (color >> 0) & 0xFF;
                    p[1] = (color >> 8) & 0xFF;
                    p[2] = (color >> 16) & 0xFF;
                    if (bpp == 4) p[3] = 0x00;
...
static void terminal_scroll_fb(void) {
    ...
    // Move pixel rows up by one character cell height
    for (int y = 0; y < (rows - 1) * ch; y++) {
        uint8_t* dest = framebuffer + (y * fb_pitch);
        uint8_t* src  = framebuffer + ((y + ch) * fb_pitch);
        memcpy(dest, src, fb_pitch);
    }

    // Clear last character row area
    for (int y = (rows - 1) * ch; y < rows * ch; y++) {
        uint8_t* row = framebuffer + (y * fb_pitch);
        for (int x = 0; x < fb_width; x++) {
            uint8_t* p = row + x * bpp;
            p[0] = (current_bg_color >> 0) & 0xFF;
            p[1] = (current_bg_color >> 8) & 0xFF;
            p[2] = (current_bg_color >> 16) & 0xFF;
            if (bpp == 4) p[3] = 0x00;
        }
    }
...
void terminal_init(void) {
    ...
    if (framebuffer_address == 0 ||
        ...
        if (framebuffer_width == 0 || framebuffer_width > 4096 ||
            ...
            // Infer bytes-per-pixel (typically 3=24bpp or 4=32bpp)
            fb_bpp = (fb_width > 0) ? (fb_pitch / fb_width) : 0;
            if (fb_bpp != 3 && fb_bpp != 4) fb_bpp = 3;
            ...
            kprintf("  Framebuffer at 0x%08x (%dx%d, pitch=%d, bpp=%d)\n",
                    framebuffer_address, fb_width, fb_height, fb_pitch, fb_bpp);
            kprintf("  Text grid: %dx%d (scale=%d)\n", cols, rows, font_scale);
...
void terminal_putchar(char c) {
    if (is_graphics_mode) {
        if (c == '\n') {
            ...
            if (cursor_x > 0) {
                cursor_x--;
                draw_char_fb(' ', cursor_x * cell_w(), cursor_y * cell_h(),
                             current_fg_color, current_bg_color);
            }
        } else {
            draw_char_fb(c, cursor_x * cell_w(), cursor_y * cell_h(),
                         current_fg_color, current_bg_color);
            cursor_x++;
        }
...
// ===== NEW: Graphics + font scale controls (for shell commands) =====
...
int terminal_set_font_scale(int scale) {
    if (!is_graphics_mode || !is_framebuffer_valid()) return -1;
...
void terminal_get_gfx_info(int* w, int* h, int* pitch, int* bpp, int* out_cols, int* out_rows) {
    if (w) *w = fb_width;
    if (h) *h = fb_height;
